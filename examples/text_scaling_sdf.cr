...
puts ">>> Available Renderers: #{SDL3.get_renderer_info.join(", ")} >>>"
puts ">>> Current Renderer: #{renderer.name} >>>"

sdf_shader = SDFShader.new(renderer, "examples/shaders/sdf_shader")

# Inside your GSDL Shader setup
info = LibSDL3::GPURenderStateCreateInfo.new
info.fragment_shader = sdf_shader.to_unsafe
info.num_sampler_bindings = 0
info.sampler_bindings = Pointer(LibSDL3::GPUTextureSamplerBinding).null
info.num_storage_textures = 0
info.storage_textures = Pointer(LibSDL3::GPUTexture*).null
info.num_storage_buffers = 0
info.storage_buffers = Pointer(LibSDL3::GPUBuffer*).null
info.props = 0_u32

sdf_render_state = LibSDL3.create_gpu_render_state(renderer.to_unsafe, pointerof(info))
...
def draw_sdf_text(renderer, texture, dest_rect, angle, sdf_render_state)
  # 1. Apply the SDF Shader
  LibSDL3.set_gpu_render_state(renderer.to_unsafe, sdf_render_state)

  # 2. Draw the texture normally
  # The GPU now uses your .msl/.spv logic to 'sharpen' the alpha
  renderer.render_texture_rotated(texture, dest_rect, angle)

  # 3. Reset to default shader
  LibSDL3.set_gpu_render_state(renderer.to_unsafe, nil)
end
...
while running
  ...
  [ {tex72, 0, 72}, {tex64, 100, 64}, {tex16, 180, 16} ].each do |tex_info|
    ...
    draw_sdf_text(renderer, tex, dest_rect, angle, sdf_render_state)
  end
